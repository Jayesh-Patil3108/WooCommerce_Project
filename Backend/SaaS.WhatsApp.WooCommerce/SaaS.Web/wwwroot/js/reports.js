document.addEventListener('DOMContentLoaded', () => {
    ...
    let reports = Array.from(tableBody.getElementsByTagName('tr')).map(row => ({
        element: row,
    ...
    const sortTable = (columnIndex, ascending = true) => {
        ...
        reports.forEach(report => tableBody.appendChild(report.element));
    };
    ...
    if (searchInput) {
        searchInput.addEventListener('input', () => {
            ...
            reports.forEach(report => {
                const matches = report.data.some(cell => cell.toLowerCase().includes(searchTerm));
                report.element.style.display = matches ? '' : 'none';
            });
        });
    }
});